...
public class Ball : MonoBehaviour
{
    ...
    [SerializeField] private GameObject powerUpLoseLifePrefab;
    [SerializeField] private GameObject powerUpBigPrefab;
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.CompareTag("block")) // Si colisionamos con un bloque
        {
            ...
            if (Mathf.Abs(rb.velocity.x) < velocityMax && Mathf.Abs(rb.velocity.y) < velocityMax)
            {
            ...
            if (random < 0.15f)
            {
                Instantiate(powerUpBigPrefab, collision.transform.position, Quaternion.identity);
            }
